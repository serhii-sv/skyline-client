...
(function($) {
  $.fn.tm_editbale = function(option, new_obj) {
    this.each(function() {
      $this = $(this);
      var i_class_name = new_obj.hasOwnProperty("iclass")
        ? new_obj.iclass
        : $this.data("iclass");
      var i_placeholder = new_obj.hasOwnProperty("iplaceholder")
        ? new_obj.iclass
        : $this.data("iplaceholder");
      var i_validate = new_obj.hasOwnProperty("ivalidate")
        ? new_obj.iclass
        : $this.data("ivalidate");
      var i_error = new_obj.hasOwnProperty("ierror")
        ? new_obj.iclass
        : $this.data("ierror");
      var i_remover = new_obj.hasOwnProperty("remover")
        ? new_obj.iclass
        : $this.data("remover");
      var i_removerclass = new_obj.hasOwnProperty("removerclass")
        ? new_obj.iclass
        : $this.data("removerclass");
      var i_date = new_obj.hasOwnProperty("date")
        ? new_obj.iclass
        : $this.data("date");
      var theme_name = new_obj.hasOwnProperty("theme")
        ? new_obj.theme
        : $this.data("theme");
      ...
      //Set FInal Settings
      $this.Global_tm.final_settings = $.extend(
        $this.Global_tm.default_settings,
        $this.Global_tm.user_settings
      );

      //Find type of element
      $this.Global_tm.final_settings.hunt_type = hunt_type();
      $this.Global_tm.final_settings.val =
        $this.Global_tm.final_settings.hunt_type["value"];

      if (option == "init") {
        if (!$this.Global_tm.already_exists) init_method();
      ...
      function init_method() {
        ...
        $this
          .addClass(
            "has_tm_editable_container " + $this.Global_tm.final_settings.theme
          )
          .attr("my_rank", Global_tm_array.length);
        ...
        //outside_btn
        var outside_btn_html =
          $this.Global_tm.final_settings.outside_btn.onshow +
          '<span class="i_edit_text">' +
          $this.Global_tm.final_settings.outside_btn.onhover +
          "</span>";
        var edit_btn = $("<a>")
          .addClass("i_edit ")
          .html(outside_btn_html);

        //outside text
        var input_field = $("<span>")
          .addClass("i_text")
          .html(make_my_clone["final_val"])
          .attr("selected_val", make_my_clone["selected_val"]);
        ...
        var i_loading = $("<div>")
          ...
          .css("display", "none")
          .html(
            '<div class="vertical0"><div class="vertical1"><i class="fa fa-spinner fa-spin"></i>&nbsp;&nbsp;Loading</div></div>'
          );

        //inside btn
        var ok_i = $("<span>").html(
          $this.Global_tm.final_settings.inside_btn.ok
        );
        var cancel_i = $("<span>").html(
          $this.Global_tm.final_settings.inside_btn.cancel
        );
        var ok_btn = $("<a>")
          .addClass("btn btn-primary waves-effect waves-light m-l-5 m-r-10")
          .append(ok_i);
        var cancel_btn = $("<a>")
          .addClass("btn btn-warning waves-effect waves-light")
          .append(cancel_i);
        var btn_container = $("<span>")
          .addClass(
            "ibtn_container " + $this.Global_tm.final_settings.inside_btn.pull
          )
          .append(ok_btn)
          .append(cancel_btn);
        ...
        var error = $("<span>")
          .addClass("error-text smg-text full_row")
          .text($this.Global_tm.final_settings.error_text);

        //remover button
        if ($this.Global_tm.final_settings.remover)
          var remover_btn = $("<a>")
            .addClass("i_remover")
            .html($this.Global_tm.final_settings.outside_btn.removebtn)
            .attr("removerclass", $this.Global_tm.final_settings.removerclass);
        else var remover_btn = "";
        var outside_btncontainer = $("<span>")
          .addClass(
            "outside_btncontainer " +
              $this.Global_tm.final_settings.outside_btn.pull
          )
          .append(edit_btn)
          .append(remover_btn);
        //attach UI
        var no_edit = $("<div>")
          .addClass("no_edit full_row")
          .append(input_field)
          .append(outside_btncontainer);
        $this
          .append(no_edit)
          .append(just_edit)
          .attr("id", "tm-edit-" + $this.Global_tm.counter);
        var template_input = $("<div>")
          .addClass("input-group full_row md-input-wrapper")
          .append(single_input)
          .append('<span class="md-line"></span>')
          .append(error);
        $($this.Global_tm.final_settings.just_edit)
          .append(i_loading)
          .append(template_input)
          ...
          switch (my_type) {
            ...
            case "checkbox":
              ...
              final_val =
                ...
                label +
                "</label> : " +
                $this.Global_tm.final_settings.hunt_type["my_text"];
              selected_val = $this.Global_tm.final_settings.val;
              break;
          ...
          if ($this.Global_tm.final_settings.full_length.outside) {
            if ($this.Global_tm.final_settings.outside_btn.new_line) {
              ...
              if ($this.Global_tm.final_settings.remover)
                outside_btn_width =
                  $(edit_btn).outerWidth() +
                  realWidth($(edit_btn).find(".i_edit_text"), theme_class) +
                  15 +
                  realWidth($(remover_btn), theme_class);
              else
                outside_btn_width =
                  $(edit_btn).outerWidth() +
                  realWidth($(edit_btn).find(".i_edit_text"), theme_class) +
      ...
      function hunt_type() {
        if (
          $this.children('input[type="text"]').length ||
          $this.children('input[type="number"]').length ||
          $this.children('input[type="email"]').length ||
          $this.children('input[type="password"]').length
        ) {
          my_type = "text";
  ...
  function remover(eve) {
    ...
    if (r) {
      ...
      var promise = Global_tm_array[
        my_rank
      ].Global_tm.final_settings.remove.before();
      promise.done(function() {
        $(eve)
          .closest(
            Global_tm_array[my_rank].Global_tm.final_settings.remover_class
          )
          .remove();
  ...
  function cancel_update(rank, eve) {
    ...
    switch (type) {
      ...
      case "radio":
        $tm_selector
          .find("input[type='radio'][value='" + shown_value + "']")
          .prop("checked", true);
        break;
      ...
      case "checkbox":
        ...
        else $tm_selector.find("input[type='checkbox']").prop("checked", false);
        break;
  ...
  function get_rank(eve) {
    return $(eve)
      .closest(".has_tm_editable_container")
      .attr("my_rank");
  }
  ...
  function get_the_values(rank) {
    ...
    return { real_value: real_val, shown_value: shown_val };
  }
})(jQuery);
